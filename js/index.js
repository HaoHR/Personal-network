	...
	for(let i=0; i<btn.length ; i++){
		...
		document.querySelector('nav .dot.step_'+ (parseInt(this.getAttribute('data-step'))+1) +'').classList.add('done')
	...
	function turn(step){
		...
		if(!!document.querySelector('#step__' + step)){
			...
			document.querySelector('#step__' + step).classList.remove('cube__face--bottom');
		}
		step+=1;
		if(!!document.querySelector('#step__' + step)){
	...
	for(var i=0; i<colors.length;i++){
		colors[i].addEventListener('click',function(){
			...
			document.querySelector('.checkbox').classList.add(this.getAttribute('data-color'));
		})
	}
		...
		inputs[i].addEventListener('keyup',function(e){
			if(this.getAttribute('type') == 'text'){
				if(this.value.length > 1 ){
					this.closest('.container').querySelector('.btn').classList.add('show');
					if(e.keyCode == 13){
						this.closest('.container').querySelector('.btn span').click();
					}
				} else {
				this.closest('.container').querySelector('.btn').classList.remove('show');
				}
			}
